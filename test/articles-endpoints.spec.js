...
describe(`Articles Endpoints`, () => {
  ...
  afterEach('cleanup', () => db('blogful_articles').truncate())
    ...
    describe(`POST /api/articles`, () => {
      ...
      it(`removes XSS attack content fromn response`, () => {
        const { maliciousArticle, expectedArticle } = makeMaliciousArticle();
          ...
          return supertest(app)
            ...
            .then(res =>
              supertest(app)
                .get(`/api/articles`)
                .expect(res => {
                  expect(res.body.title).to.eql(expectedArticles.title)
                  expect(res.body.content).to.eql(expectedArticles.content)
                })
            )
        })
      })
          ...
          return supertest(app)
            ...
            .then(res => 
                ...
                .expect(res => {
                  expect(res.body.title).to.eql(expectedArticle.title)
                  expect(res.body.content).to.eql(expectedArticle.content)
                  expect(Object.entries(res.body).length).to.eql(Object.entries(expectedArticle).length)
                })
              )
        })
      })
